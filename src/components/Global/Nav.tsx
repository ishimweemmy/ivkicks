...
const Nav: FC<NavProps> = (props) => {
  ...
  return (
    <nav
      className={`w-auto h-[6rem] sm:px-4 flex items-center justify-around px-[7rem] gap-[12rem] lg:gap-[4rem] lg:px-[2rem] xl:px-[2rem] md:justify-between md:px-8 lsm:gap-[25rem] xMiniTablet:gap-[30rem] miniTablet:px-4 transition-all overflow-hidden z-30 lssm:justify-between lssm:px-4 sm:gap-0 xl:pl-[5rem] xs:gap-2 xs:p-0 xs:pl-4 xs:justify-between 2xl:pl-[6rem] duration-700`}
      id="top"
    ...
    >
      ...
      <div className="min-w-[10rem] lg:w-[20rem] h-[4rem] sm:min-w-[8rem] xs:gap-6 bg-[bue] bg-blue] flex items-center justify-center gap-8 lg:gap-[3rem] sm:gap-8 transition-all">
        <FiSearch
          className={`w-[1.3rem] xs:w-[1rem] lg:w-[2rem] miniTablet:w-[3rem] miniTablet:h-[1.5rem] lg:h-[1.5rem] h-[1.3rem] sm:w-[1rem] block md:hidden lg:block font-black text-${location.pathname == "/about" || location.pathname == "/products/promoProducts" ? "white" : "black"
            }`}
        />
        ...
        <Link to="/cart" className="w-fit h-fit relative md:hidden lg:block">
          <FiShoppingBag
            className={`w-[1.5rem] xs:w-[1rem] sm:w-[1rem] miniTablet:w-[1.75rem] lg:w-[2rem] h-[1.5rem] font-bold ${location.pathname == "/about" || location.pathname == "/products/promoProducts" ? "text-white" : "black"
              }`}
          />
